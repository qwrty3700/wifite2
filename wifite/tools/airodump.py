    ...
    def __new__(cls, interface=None, *args, **kwargs):
        ...
        if interface and interface.startswith('rtwmon-'):
            try:
                ...
                return _RtwmonAirodumpFallback(interface, *args, **kwargs)
        
        return super(Airodump, cls).__new__(cls)


class _RtwmonAirodumpFallback(Dependency):
    dependency_required = True
    dependency_name = 'rtwmon'
    dependency_url = 'https://github.com/ahdiotllfer/rtwmon'
    ...
    def __init__(self, interface=None, channel=None, encryption=None,
        ...
        self.channel = channel if channel is not None else Configuration.target_channel
        ...
        self.targets = []
        self._targets_by_bssid = {}
        self._clients_by_bssid = {}
        self._stdout_buf = b""

        self._rtwmon_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'rtwmon', 'rtwmon.py'))
        self._bus = None
        self._addr = None
        m = re.fullmatch(r"rtwmon-([0-9]+)-([0-9]+)", str(self.interface).strip())
        if m:
            try:
                self._bus = int(m.group(1))
                self._addr = int(m.group(2))
            except Exception:
                self._bus = None
                self._addr = None

    def __enter__(self):
        if not os.path.exists(self._rtwmon_path):
            raise Exception(f"rtwmon.py not found at {self._rtwmon_path}")

        cmd = [
            'python3', '-u', self._rtwmon_path,
        ]
        if self._bus is not None and self._addr is not None:
            cmd += ['--bus', str(int(self._bus)), '--address', str(int(self._addr))]

        cmd += ['scan', '--forever']
        if self.channel:
            cmd += ['--channels', str(self.channel)]
        else:
            cmd += ['--channels', '1-13']

        self.command = cmd
        self.pid = Process(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        try:
    ...
    def _handle_line(self, line: str):
        ...
        m_ap = re.search(r"\bch=(\d+)\b.*\bbssid=([0-9a-f:]{17})\b.*\bseen=(\d+)\b.*\benc=([A-Z0-9]+)\b.*\bssid=(.*)$", line, re.IGNORECASE)
        if m_ap:
            ch = int(m_ap.group(1))
            bssid = m_ap.group(2).lower()
            seen = int(m_ap.group(3))
            enc = str(m_ap.group(4)).upper()
            ssid = str(m_ap.group(5))
            if ssid == "<hidden>":
                ssid = ""
            ...
            tgt = self._targets_by_bssid.get(bssid)
            if tgt is None:
                try:
                    tgt = Target(fields)
                except Exception:
                    return
                self._targets_by_bssid[bssid] = tgt
            else:
                try:
                    tgt.channel = int(ch)
                except Exception:
                    pass
                try:
                    tgt.beacons = int(seen)
                except Exception:
                    pass
                try:
                    tgt.essid = ssid if ssid else None
                    tgt.essid_known = bool(ssid)
                    tgt.essid_len = int(len(ssid))
                except Exception:
                    pass

            clients = self._clients_by_bssid.get(bssid, {})
            tgt.clients = list(clients.values())
            return

        m_sta = re.search(r"\bch=(\d+)\b.*\bsta=([0-9a-f:]{17})\b.*\bbssid=([0-9a-f:]{17})\b.*\bseen=(\d+)\b", line, re.IGNORECASE)
        if m_sta:
            bssid = m_sta.group(3).lower()
            sta = m_sta.group(2).lower()
            seen = int(m_sta.group(4))
            by_bssid = self._clients_by_bssid.get(bssid)
            if by_bssid is None:
                by_bssid = {}
                self._clients_by_bssid[bssid] = by_bssid
            if sta not in by_bssid:
                fields = [sta, "", "", "-30", str(int(seen)), bssid, ""]
                try:
                    by_bssid[sta] = Client(fields)
                except Exception:
                    return
            tgt = self._targets_by_bssid.get(bssid)
            if tgt is not None:
                tgt.clients = list(by_bssid.values())
            return
    ...
    def get_targets(self, old_targets=None, apply_filter=True, target_archives=None):
        ...
        self._drain_stdout()
        new_targets = list(self._targets_by_bssid.values())
        ...
        if apply_filter:
            new_targets = Airodump.filter_targets(new_targets, skip_wps=True)
